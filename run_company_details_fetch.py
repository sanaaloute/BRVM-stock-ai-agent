...
def main() -> int:
    ...
    refreshed = skipped = failed = fetched = 0
    for symbol in symbols:
        ...
        try:
            data = fetch_and_save_company_details(symbol, country_code)
    ...
    logger.info(
        "Summary: %d refreshed, %d skipped, %d failed (of %d symbols)",
        refreshed, skipped, failed, len(symbols),
    )
