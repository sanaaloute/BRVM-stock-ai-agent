...
def test_cleanup_stale_threads():
    ...
    original_user_db_path = user_db.DB_PATH
    original_cp = chat_mod._checkpointer
    original_ttl = config.MEMORY_TTL_HOURS
    try:
        # thread_activity now lives in the shared user DB (app/db), not the
        # checkpoint DB: rebind user_db.DB_PATH to isolate this test.
        user_db.DB_PATH = tmpdir / "shared_test.db"
        user_db.init_db()
        ...
        with db_engine.session_scope() as s:
            s.add(db_models.ThreadActivity(
                thread_id="stale-thread", last_seen=now - 48 * 3600
            ))
            s.add(db_models.ThreadActivity(thread_id="fresh-thread", last_seen=now))
        ...
        with db_engine.session_scope() as s:
            rows = [r[0] for r in s.execute(select(db_models.ThreadActivity.thread_id))]
        assert rows == ["fresh-thread"], rows
    finally:
        user_db.DB_PATH = original_user_db_path
